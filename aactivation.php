...
if(isset($_GET['token'])){
    ...
    if($query){
        if(isset($_SESSION['msg'])){
            ...
            ?>
            <script>
                location.replace("aactivation.php?toke=<?php echo $token?>");
            </script>
            <?php
        }else{
            $_SESSION['msg'] = "You are logged out.";
            header('location:aactivation.php');
        }
        }else{
...
<div class="content">
  <h3>FOODIES Patner</h3>
  <p>Welcome New Partner Activation of your Mobile number and Email.</p>
...
<div class="row">
  ...
  <div class="form-group">
    <div class="form-check">
      ...
      <label class="form-check-label" for="gridCheck">
      Resend OTP <a href="aforgotpass.php?token=<?php echo $token?>">Click </a> here.
      </label>
    </div>
  </div><br>
  <div class="row">
...
if(isset($_POST['submit']))
{
    ...
    $emailquery =" select * from signuphotel where token ='$token'  and otp ='$otp' and status='Active'";
$query = mysqli_query($con, $emailquery);
$emailcount = mysqli_num_rows($query);
if($emailcount)
{
    $updatequery = "update signuphotel set mstatus='Active' where token='$token' ";
