...
if(isset($_GET['cateid'])){
    $cateid = $_GET['cateid'];
    $selectquery = "SELECT * FROM categories where cateid='$cateid'";
  $query1 = mysqli_query($con,$selectquery);
  while($result1 =mysqli_fetch_array($query1)){
  $cat=$result1['categories'];
  }
?>
<html>
      <head>
  ...
  <nav class="navbar fixed-top navbar-expand-lg navbar-light bg-light">
    ...
    <a class="navbar-brand" href="#">&nbsp;
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
          <a class="nav-link   active" aria-current="page" href="ucata.php">Categories</a>
        </li>&nbsp;&nbsp;&nbsp;&nbsp;
        <li class="nav-item">
    ...
    <?php 
      ...
      <form class="d-flex" action ="uhome1.php" method ="POST">
        ...
        <input class="form-control me-2" type="search" name="food" placeholder="Search Food Name.." value="<?php echo $cat ?>"aria-label="Search">
  ...
  <?php
    ...
    $selectquery = "SELECT * FROM menu INNER JOIN categories on categories.cateid=menu.cateid INNER JOIN signuphotel on signuphotel.resid=menu.resid where city='$city' and status ='Active' and menu.cateid='$cateid' group by menu.resid";
  $query = mysqli_query($con,$selectquery);
  while($result =mysqli_fetch_array($query)){
      ?>
    ...
    $t=$result['hstatus'];
    if($t=="Closed"){?>
      <a href ="uhotel1.php?resid=<?php echo $result['resid'];?>"><img src="<?php echo $result['image'];?>" class="card-img-top" alt="<?php echo $result['hotel'];?>" style="filter: grayscale(100%);width: 343;height:200;padding:2px;display:flex;"></a>
    <?php } else { ?>
    <a href ="uhotel1.php?resid=<?php echo $result['resid'];?>"><img src="<?php echo $result['image'];?>" class="card-img-top" alt="<?php echo $result['hotel'];?>" style="width: 343;height:200;padding:2px;display:flex;"></a>
    <?php
    }?>
    <div class="card-body">
    <div class="row">
  ...
  $search = mysqli_real_escape_string($con, $_POST['food']);
?>
<html>
      <head>
          <title>FOODIES</title>
  ...
  <nav class="navbar fixed-top navbar-expand-lg navbar-light bg-light">
    ...
    <?php 
      ...
      <form class="d-flex" action ="uhome1.php" method ="POST">
        ...
        <form class="d-flex" action ="ucata1.php" method ="POST">
        <input class="form-control me-2" type="search" name="food" placeholder="Search Food Name.." value="<?php echo $search ?>" aria-label="Search">
        <button class="btn btn-outline-success" name="submit" type="submit"><i class ="fa fa-search"></i></button>
      </form>&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="ulogout.php"><button class="btn btn-outline-danger" type="submit"><i class ="fa fa-sign-out"></i></button></a>&nbsp;&nbsp;&nbsp;&nbsp;
        <a href ="ucustomerprofile.php"><img src="<?php echo $image ?>" alt="" style="width:50px;height:50; border-radius: 50%;"></a>
  ...
  <?php
    ...
    $selectquery = "SELECT menu.image,signuphotel.hotel,signuphotel.hstatus,signuphotel.Categories FROM menu INNER JOIN categories on categories.cateid=menu.cateid INNER JOIN signuphotel on signuphotel.resid=menu.resid where city='$city' and status ='Active' and menu.foodname='$search' group by menu.resid";
  $query = mysqli_query($con,$selectquery);
  while($result =mysqli_fetch_array($query)){
      ?>
      <div class="col-sm-3" style="padding:10px;">
