...
if(isset($_GET['token'])){
    $token = $_GET['token'];
$otp =rand(1000,9999);
$updatequery = "update signuphotel set otp='$otp' where token='$token' ";

    $query = mysqli_query($con, $updatequery);

    if($query){$emailquery =" select * from signuphotel where token='$token' ";
...
$subject = "Foodies Partner New OTP";
$headers = "From: Foodiesmitaoe@gmail.com\r\n";
          ...
          * { margin: 0; padding: 0; }
          ...
          $sender_email ="From: Foodiesmitaoe@gmail.com";
        
          if(mail($email, $subject, $message,$headers, $sender_email))
          {
  $_SESSION['msg'] ="Check your mail $email New OTP has Send." ;
  ?>
  <script>location.replace("aactivation.php?toke=<?php echo $token?>");</script>";
  <?php
  }else{
    ?>
    <script>
